...
def load_biomass_data(year=None, year_end=None, folder="interim", shape=None, resolution=250, delta=None, name=None):
    ...
    if delta is not None:
        ...
        bio_data = torch.nn.functional.pad(bio_data, (diff_x, int(diff_x+mod_x), diff_y, int(diff_y+mod_y)), value=nodata)
...
def build_features(output_px=40, input_px=400, resolution=250):
    ...
    processed_path = f'data/processed_transition/biomass/amazonia/{resolution}m/'
    ...
    for bio_data_name in bio_data_folder:
        ...
        torch.save(bio_data, f'data/processed_transition/biomass/amazonia/{resolution}m/{bio_data_name}.pt')

    dtype = np.int16 if np.max(bio_data_2010.shape) <= 32767 else np.int32
    torch.save(torch.from_numpy(train_data_points.astype(dtype)), 'data/processed_transition/train_data.pt')
    torch.save(torch.from_numpy(val_data_points.astype(dtype)), 'data/processed_transition/val_data.pt')
    torch.save(torch.from_numpy(test_data_points.astype(dtype)), 'data/processed_transition/test_data.pt')
